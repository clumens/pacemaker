...
int32_t pcmk__daemon_ipc_accept(pcmk__daemon_t *srv, qb_ipcs_connection_t *c,
                                uid_t uid, gid_t gid);
void pcmk__daemon_ipc_cleanup(pcmk__daemon_t *srv);
int32_t pcmk__daemon_ipc_closed(pcmk__daemon_t *srv, qb_ipcs_connection_t *c);
void pcmk__daemon_ipc_created(pcmk__daemon_t *srv, qb_ipcs_connection_t *c);
void pcmk__daemon_ipc_destroy(pcmk__daemon_t *srv, qb_ipcs_connection_t *c);
void pcmk__daemon_ipc_dispatch(pcmk__daemon_t *srv, qb_ipcs_connection_t *c,
                               void *data, size_t size);
bool pcmk__daemon_ipc_running(pcmk__daemon_t *srv);
int pcmk__daemon_ipc_init(pcmk__daemon_t *srv);
bool pcmk__generic_ipc_running(pcmk__daemon_t *srv);

// Mainloop management functions

int pcmk__daemon_init(pcmk__daemon_t *srv, const pcmk__server_command_t handler[]);
