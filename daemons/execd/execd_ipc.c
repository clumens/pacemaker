...
void
execd_ipc_created(pcmk__daemon_t *srv, pcmk__client_t *client)
{
...
void
execd_ipc_closed(pcmk__daemon_t *srv, pcmk__client_t *client)
{
...
void
execd_ipc_dispatch(pcmk__daemon_t *srv, pcmk__request_t *request)
{
    request->op = pcmk__xe_get_copy(request->xml, srv->op);
