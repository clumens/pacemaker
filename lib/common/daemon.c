...
{
    pcmk__daemon_t *srv = qb_ipcs_connection_service_context_get(c);
    return pcmk__daemon_ipc_accept(srv, c, uid, gid);
}
...
{
    pcmk__daemon_t *srv = qb_ipcs_connection_service_context_get(c);
    return pcmk__daemon_ipc_closed(srv, c);
}
...
{
    pcmk__daemon_t *srv = qb_ipcs_connection_service_context_get(c);
    pcmk__daemon_ipc_created(srv, c);
}
...
{
    pcmk__daemon_t *srv = qb_ipcs_connection_service_context_get(c);
    pcmk__daemon_ipc_destroy(srv, c);
}
...
{
    pcmk__daemon_t *srv = qb_ipcs_connection_service_context_get(c);
    pcmk__daemon_ipc_dispatch(srv, c, data, size);
    return 0;
}
 ...
 */
int32_t
pcmk__daemon_ipc_accept(pcmk__daemon_t *srv, qb_ipcs_connection_t *c,
                        uid_t uid, gid_t gid)
{
 ...
 */
void
pcmk__daemon_ipc_cleanup(pcmk__daemon_t *srv)
{
    pcmk__drop_all_clients(srv->ipcs);
    g_clear_pointer(&srv->ipcs, qb_ipcs_destroy);
 ...
 */
int32_t
pcmk__daemon_ipc_closed(pcmk__daemon_t *srv, qb_ipcs_connection_t *c)
{
    ...
    if (srv->ipc_fns->closed != NULL) {
        srv->ipc_fns->closed(srv, client);
    } else {
 ...
 */
void
pcmk__daemon_ipc_created(pcmk__daemon_t *srv, qb_ipcs_connection_t *c)
{
    pcmk__client_t *client = pcmk__find_client(c);
    ...
    if (srv->ipc_fns->created != NULL) {
        srv->ipc_fns->created(srv, client);
    }
}
 ...
 */
void
pcmk__daemon_ipc_destroy(pcmk__daemon_t *srv, qb_ipcs_connection_t *c)
{
    pcmk__trace("Destroying client connection %p", c);
    pcmk__daemon_ipc_closed(srv, c);
}
 ...
 */
void
pcmk__daemon_ipc_dispatch(pcmk__daemon_t *srv, qb_ipcs_connection_t *c,
 ...
 */
int
pcmk__daemon_ipc_init(pcmk__daemon_t *srv)
{
    pcmk__assert(srv->ipcs == NULL);

    srv->ipcs = mainloop_add_ipc_server_with_prio(pcmk__server_ipc_name(srv->type),
    ...
    qb_ipcs_service_context_set(srv->ipcs, srv);
