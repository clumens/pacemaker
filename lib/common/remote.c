...
pcmk__remote_send_xml(pcmk__remote_t *remote, const xmlNode *msg)
{
    ...
    if ((UINT32_MAX - iov[0].iov_len) < payload_len) {
    ...
    if (payload_len < PCMK__BZ2_THRESHOLD) {
        ...
        if (pcmk__compress(xml_text->str, payload_len, 0, &compressed,
                           &new_size) == pcmk_rc_ok) {
...
pcmk__remote_message_xml(pcmk__remote_t *remote)
{
    ...
    if (header->version != REMOTE_MSG_VERSION) {
        pcmk__err("Header version %d does not match expected version %d",
                  header->version, REMOTE_MSG_VERSION);
        return NULL;
    }
